...
const {mutate: createTask} = useMutation({
    mutationFn: async ({taskDescription}: { taskDescription: string }) => postTask({
...
<template>
    <div>
        <div style="position: relative">
            <input ref="inputRef" v-model.trim="filter" placeholder="Find or add a task"
                   style="width: 100%; font-size: 1.2em; padding-right: 2.8em"
                   type="text"
                   @keydown.enter="createTask({ taskDescription: filter})"/>
            <button
                style="font-weight: bold; position: absolute; background: #747bff22; color: #747bff; font-size: large; padding: 0.25em 0.5em; right: 0.2em; top:0.2em; line-height: 1em"
        ...
        <h3 style="display: flex; justify-content: center">
            <Transition mode="out-in">
                ...
                <div v-else-if="!isPending">Found
                    {{ tasks.meta.pagination.total }} tasks
                </div>
            </Transition>
        </h3>
        <Transition mode="out-in">
